...
export class RelatoriosService {
  async getStudentsReport(filters: any = {}) {
    const query = db('students as s')
      .join('users as u', 'u.id', 's.user_id')
      .join('enrollments as e', 'e.student_id', 's.id')
      .join('classes as c', 'c.id', 'e.class_id')
      .select('s.id', 's.student_number', 'u.first_name', 'u.last_name', 'u.email', 's.birth_date', 's.gender', 'c.name as class_name', 'e.status as enrollment_status');

    if (filters.school_id) query.where('s.school_id', filters.school_id);
  ...
  async getAttendanceReport(filters: any = {}) {
    const query = db('attendance_records as ar')
      ...
      .select('s.student_number', 'u.first_name', 'u.last_name', 'c.name as class_name', 'sub.name as subject_name', 'ar.date', 'ar.status');
  ...
  async getGradesReport(filters: any = {}) {
    const query = db('grades as g')
      ...
      .select('s.student_number', 'u.first_name', 'u.last_name', 'c.name as class_name', 'sub.name as subject_name', 'a.name as assessment_name', 'at.name as type_name', 'g.score', 'at.max_score', 'a.trimester');
  ...
  async getFinancialReport(filters: any = {}) {
    const query = db('student_fees as sf')
      ...
      .select('s.student_number', 'u.first_name', 'u.last_name', 'ft.name as fee_type', 'sf.amount', 'sf.due_date', 'sf.status')
  ...
  async getEnrollmentsReport(filters: any = {}) {
    const query = db('enrollments as e')
      .join('students as s', 's.id', 'e.student_id')
      .join('users as u', 'u.id', 's.user_id')
      .join('classes as c', 'c.id', 'e.class_id')
      .join('academic_years as ay', 'ay.id', 'e.academic_year_id')
      .select('s.student_number', 'u.first_name', 'u.last_name', 'c.name as class_name', 'ay.name as academic_year', 'e.enrollment_date', 'e.status');
