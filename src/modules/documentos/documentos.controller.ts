...
export class DocumentosController {
  async listTemplates(req: AuthRequest, res: Response) {
    try {
      const templates = await service.listTemplates(req.query.school_id as string);
  ...
  async listDocuments(req: AuthRequest, res: Response) {
    try {
      ...
      const result = await service.listDocuments(Number(page), Number(limit), filters);
  ...
  async requestDocument(req: AuthRequest, res: Response) {
    try {
      const document = await service.requestDocument({ ...req.body, requested_by: req.user!.id });
  ...
  async updateDocumentStatus(req: AuthRequest, res: Response) {
    try {
      ...
      const document = await service.updateDocumentStatus(req.params.id, status, notes);
