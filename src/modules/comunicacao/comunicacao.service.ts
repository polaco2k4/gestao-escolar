...
export class ComunicacaoService {
  async listMessages(userId: string, page = 1, limit = 20) {
    const { offset } = paginate(page, limit);
    const messages = await db('message_recipients as mr')
  ...
  async sendMessage(senderId: string, data: { subject: string; body: string; recipient_ids: string[]; priority?: string; school_id: string }) {
